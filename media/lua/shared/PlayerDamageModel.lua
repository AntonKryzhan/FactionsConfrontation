...
local function pdm_compatProvider()
    return NPCCompatibilityBridge or NPCLegacyGlobalsBridge.Get("Compatibility")
end
...
function PlayerDamageModel.BulletHit(shooter, player)
    ...
    local item = pdm_compatProvider().InstanceItem("Base.AssaultRifle2")
    ...
    if isSuperficial then
        ...
        player:addBlood(0.2)
        pdm_compatProvider().Splash(player, item, shooter)
    else
