NPCActionUnbarricadeMetalBridge = NPCActionUnbarricadeMetalBridge or {}
...
function NPCActionUnbarricadeMetalBridge.OnStart(zombie, task)
    ensureTool(zombie, "Base.BlowTorch")
    playSound(zombie, "BlowTorch")
    return true
end

function NPCActionUnbarricadeMetalBridge.OnWorking(zombie, task)
...
function NPCActionUnbarricadeMetalBridge.OnComplete(zombie, task)
    stopSounds(zombie)
    playSound(zombie, "RemoveBarricadeMetal")

    if isController(zombie) then
        sendUnbarricade(task)
        dropItem(zombie, "Base.SheetMetal")
    end
