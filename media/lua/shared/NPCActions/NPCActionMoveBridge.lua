NPCActionMoveBridge = NPCActionMoveBridge or {}
...
local function IsMoveTaskValid(zombie, task)
...
local function BeginMotion(zombie, task)
    if not IsMoveTaskValid(zombie, task) then return end
...
local function StartPath(zombie, task, useFloatPath)
    if not IsController(zombie) or not IsMoveTaskValid(zombie, task) then return end
    local dx, dy = GetMoveDistance(zombie, task)
...
local function UpdatePath(zombie, task)
    if not IsController(zombie) then return false end
...
local function FallbackWorking(zombie, task)
...
local function FinishMove(zombie, task)
...
NPCActionMoveBridge.OnStart = function(zombie, task)
    if not IsMoveTaskValid(zombie, task) then return true end
    if NPCMovementStabilityBridge and NPCMovementStabilityBridge.Prepare then
        if NPCMovementStabilityBridge.Prepare(zombie, task, "Move") == false then
    ...
    NormalizeWalkType(zombie, task)
    BeginMotion(zombie, task)
    StartPath(zombie, task, false)
    return true
end

NPCActionMoveBridge.OnWorking = function(zombie, task)
    if not IsMoveTaskValid(zombie, task) then return true end
    NormalizeWalkType(zombie, task)
    if UpdatePath(zombie, task) then return true end
    return FallbackWorking(zombie, task)
end

NPCActionMoveBridge.OnComplete = function(zombie, task)
    return FinishMove(zombie, task)
end
