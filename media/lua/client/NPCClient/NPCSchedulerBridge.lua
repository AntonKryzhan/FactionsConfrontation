...
require "NPCBases/NPCBaseGroupPlacementsBridge"
require "NPCClient/NPCEventMarkerHandler"
require "NPCClient/NPCBroadcasterBridge"
require "NPCCore/NPCLegacyContractBridge"
require "NPCCore/NPCLegacyGlobalsBridge"

NPCSchedulerBridge = NPCSchedulerBridge or {}

NPCBaseScenes = NPCBaseScenes or {}

local NPC_SCHEDULER_LEGACY_SANDBOX = NPCLegacyContractBridge.Sandbox.main
local NPC_SCHEDULER_LEGACY_PROGRAMS = {
    bandit = NPCLegacyContractBridge.Keys.FLAG
}

local function bs_sandboxValue(name, defaultValue)
    local vars = SandboxVars and SandboxVars[NPC_SCHEDULER_LEGACY_SANDBOX] or nil
    if vars and vars[name] ~= nil then return vars[name] end
    return defaultValue
end

local function bs_sandboxBool(name, defaultValue)
    local value = bs_sandboxValue(name, defaultValue == true)
    return value == true or value == 1 or value == "true"
end

local function bs_isHostileArrivalProgram(programName)
    return programName == "Raider" or programName == NPC_SCHEDULER_LEGACY_PROGRAMS.bandit
end
...
local function bs_legacyFactionProfilesEnabled()
    return false
end

local function bs_allowWorldDecoration()
    return false
end
...
function NPCSchedulerBridge.DaysSinceApo()
...
function NPCSchedulerBridge.GetWaveDataAll()
    local waveCnt = 16
    local waveData = {}

    if not bs_legacyFactionProfilesEnabled() then
        return waveData
    end
    for i=1, waveCnt do
        local wave = {}

        wave.enabled = bs_sandboxValue("Clan_" .. tostring(i) .. "_WaveEnabled")
        wave.enemyBehaviour = bs_sandboxValue("Clan_" .. tostring(i) .. "_EnemyBehaviour")
        wave.firstDay = bs_sandboxValue("Clan_" .. tostring(i) .. "_FirstDay")
        wave.lastDay = bs_sandboxValue("Clan_" .. tostring(i) .. "_LastDay")
        wave.spawnDistance = bs_sandboxValue("Clan_" .. tostring(i) .. "_SpawnDistance")
        wave.spawnHourlyChance = bs_sandboxValue("Clan_" .. tostring(i) .. "_SpawnHourlyChance")
        wave.groupSize = bs_sandboxValue("Clan_" .. tostring(i) .. "_GroupSize")
        wave.clanId = bs_sandboxValue("Clan_" .. tostring(i) .. "_GroupName")
        wave.hasPistolChance = bs_sandboxValue("Clan_" .. tostring(i) .. "_HasPistolChance")
        wave.pistolMagCount = bs_sandboxValue("Clan_" .. tostring(i) .. "_PistolMagCount")
        wave.hasRifleChance = bs_sandboxValue("Clan_" .. tostring(i) .. "_HasRifleChance")
        wave.rifleMagCount = bs_sandboxValue("Clan_" .. tostring(i) .. "_RifleMagCount")
...
function NPCSchedulerBridge.GetWaveDataForDay(day)
    local waveData = NPCSchedulerBridge.GetWaveDataAll()
...
function NPCSchedulerBridge.GenerateSpawnPoint(player, d)
    local spawnPoints = {}
    ...
    local function isTooCloseToPlayer(x, y)
        ...
        for i=0, playerList:size()-1 do
            ...
            if player and not NPCPlayerClient.IsGhost(player) then
                local dist = NPCUtils.DistTo(x, y, player:getX(), player:getY())
                if dist < 30 then
                    return true
                end
            end
...
function NPCSchedulerBridge.SpawnWave(player, wave)
    local event = {}
    event.occured = false
    event.program = {}
    ...
    if wave.enemyBehaviour == 1 then
        event.program.name = NPCUtils.Choice({"Raider", "Looter", "BaseGuard", "Thief"})
    ...
    elseif wave.enemyBehaviour == 6 then
        event.program.name = NPCUtils.Choice({"Looter", "BaseGuard", "Thief"})        
    ...
    elseif wave.enemyBehaviour == 9 then
        event.program.name = NPCUtils.Choice({"Looter", "Companion"})
        event.hostile = false
    ...
    local spawnPoint = NPCSchedulerBridge.GenerateSpawnPoint(player, wave.spawnDistance)
    if spawnPoint then

        for i=1, wave.groupSize do
            local bandit = NPCCreatorBridge.MakeFromWave(wave)
            table.insert(event.bandits, bandit)
        end
    
        if #event.bandits > 0 then
            print ("[INFO] Spawning a bandit group against player id: " .. NPCUtils.GetCharacterID(player))
            event.x = spawnPoint.x
            event.y = spawnPoint.y

            -- Custom arrival audio is intentionally disabled in the neutral sound-strip stage.

            if bs_sandboxBool("General_ArrivalWakeUp", true) and event.hostile and bs_isHostileArrivalProgram(event.program.name) then
                NPCPlayerClient.WakeEveryone()
            end
            -- player:Say("legacy NPCs are coming!")

            if bs_sandboxBool("General_ArrivaPanic", true) and event.hostile and bs_isHostileArrivalProgram(event.program.name) then
                local stats = player:getStats()
                stats:setPanic(80)
            end

            -- road block spawn
            if bs_sandboxBool("General_BuildRoadblock", true) then
                ...
                if event.hostile and spawnPoint.groundType == "street" and vehicleCount < 7 then
                    ...
                    if allfree then
                        ...
                        local args = {type=NPCUtils.Choice(carOpts), x=spawnPoint.x-ym*3, y=spawnPoint.y-xm*3, engine=true, lights=true, lightbar=true}
                        sendClientCommand(player, 'NPCCommands', 'VehicleSpawn', args)
                        
                        if bs_allowWorldDecoration() then
                            for b=-4, 4, 2 do
                                NPCBasePlacementsBridge.IsoObject(sprite, spawnPoint.x + xm * b, spawnPoint.y + ym * b, 0)
                            end
                        end
                    end
                end
            ...
            if bs_sandboxBool("General_ArrivalIcon", true) then
                local color
                local icon
                local desc
                if (event.program.name == "Raider" or event.program.name == NPC_SCHEDULER_LEGACY_PROGRAMS.bandit) then 
                ...
                NPCEventMarkerHandler.setOrUpdate(getRandomUUID(), icon, 10, event.x, event.y, color, desc)
            end
        end
    end  
end

function NPCSchedulerBridge.RaiseDefences(x, y)
    local cell = getCell()
    ...
    if building then
        ...
        if buildingDef then
            ...
            NPCBaseGroupPlacementsBridge.Junk(x, y, 0, w, h, 3)
            if ZombRand(5) == 0 then    
                NPCBaseGroupPlacementsBridge.Item(NPCCompatibilityBridge.GetLegacyItem("Base.WineOpen"), x, y, 0, w, h, 2)
                NPCBaseGroupPlacementsBridge.Item(NPCCompatibilityBridge.GetLegacyItem("Base.BeerCanEmpty"), x, y, 0, w, h, 2)
                NPCBaseGroupPlacementsBridge.Item(NPCCompatibilityBridge.GetLegacyItem("Base.ToiletPaper"), x, y, 0, w, h, 1)
                NPCBaseGroupPlacementsBridge.Item(NPCCompatibilityBridge.GetLegacyItem("Base.TinCanEmpty"), x, y, 0, w, h, 2)
            end
            ...
            if genSquare then
                ...
                if generator then
                    ...
                    local genItem = NPCCompatibilityBridge.InstanceItem("Base.Generator")
                                ...
                                if object then
                                    ...
                                    if z == 0 then
                                        if instanceof(object, "IsoWindow") then
                                            ...
                                            sendClientCommand(getPlayer(), 'NPCCommands', 'Barricade', args)
                                        end
                                    end

                                    local lootAmount = (tonumber(bs_sandboxValue("General_DefenderLootAmount", 5)) or 5) - 1
                                    ...
                                    if lootAmount > 0 and roomCnt > 2 and c < maxc then
                                        ...
                                        if fridge then
                                            NPCLootBridge.FillContainer(fridge, NPCLootBridge.FreshFoodItems, lootAmount)
                                            c = c + 1
                                        end

                                        local freezer = object:getContainerByType("freezer")
                                        if freezer then
                                            NPCLootBridge.FillContainer(freezer, NPCLootBridge.FreshFoodItems, lootAmount)
                                            c = c + 1
                                        end
                                        ...
                                        if ZombRand(10) == 1 then
                                            ...
                                            if counter then
                                                NPCLootBridge.FillContainer(counter, NPCLootBridge.CannedFoodItems, lootAmount)
                                                c = c + 1
                                            end

                                            local crate = object:getContainerByType("crate")
                                            if crate then
                                                NPCLootBridge.FillContainer(crate, NPCLootBridge.CannedFoodItems, lootAmount)
                                                c = c + 1
                                            end
                                        end
                                    end
...
function NPCSchedulerBridge.GenerateSpawnPointsInRandomBuilding(character, min, max)
    ...
    local function checkBuilding(building)
        ...
        local bid = NPCUtils.GetBuildingID(buildingDef)
...
function NPCSchedulerBridge.GetDensityScore(player, r)
    local score = 0
    local px = player:getX()
    local py = player:getY()
    ...
    -- local numBuildings = legacy utility API for nearby-building counts
...
function NPCSchedulerBridge.GetSpawnZoneBoost(player, clanId)
    local zoneBoost = 1
    local clan = nil
    if NPCCreatorBridge and NPCCreatorBridge.GetClanFromWave then
        clan = NPCCreatorBridge.GetClanFromWave({clanId = clanId})
    elseif NPCCreatorBridge and NPCCreatorBridge.GroupMap then
        clan = NPCCreatorBridge.GroupMap[clanId]
    end
...
function NPCSchedulerBridge.SpawnDefenders(player, min, max)
    local event = {}
    event.hostile = true
    event.occured = false
    event.program = {}
    ...
    local spawnData = NPCSchedulerBridge.GenerateSpawnPointsInRandomBuilding(player, min, max)
    ...
    NPCSchedulerBridge.RaiseDefences(spawnData.buildingCoords.x, spawnData.buildingCoords.y)
    ...
    for _, spawnPoint in pairs(spawnData.spawnPoints) do
        ...
        local bandit = NPCCreatorBridge.MakeFromSpawnType(spawnData)
        if bandit then
        ...
        cnt = cnt + 1
        if cnt > (tonumber(bs_sandboxValue("General_DefenderMaxOccupants", 16)) or 16) then break end
    end
    ...
    if bs_sandboxBool("General_ArrivalIcon", true) then
        ...
        NPCEventMarkerHandler.setOrUpdate(getRandomUUID(), icon, 10, spawnData.buildingCenterCoords.x, spawnData.buildingCenterCoords.y, color, desc)
    end
end

function NPCSchedulerBridge.SpawnBase(player, sceneNo)
    if not bs_allowWorldDecoration() then return end
    if type(NPCBaseScenes) ~= "table" or type(NPCBaseScenes[sceneNo]) ~= "function" then return end
    local cell = getCell()
    local spawnPoint = NPCSchedulerBridge.GenerateSpawnPoint(player, ZombRand(40,70))
    if spawnPoint then
        local canPlace = NPCBaseGroupPlacementsBridge.CheckSpace(spawnPoint.x-10, spawnPoint.y-10, 40, 40)
        if canPlace then
            print ("[INFO] Spawning a camp site " .. tostring(sceneNo) .. " for player id: " .. NPCUtils.GetCharacterID(player) .. ".")
            local square = cell:getGridSquare(spawnPoint.x, spawnPoint.y, 0)
            NPCBaseScenes[sceneNo](player, square)

            if bs_sandboxBool("General_ArrivalIcon", true) then
                color = {r=1, g=0, b=0}
                local desc = "Hostile Camp"
                NPCEventMarkerHandler.setOrUpdate(getRandomUUID(), "media/ui/tent.png", 10, spawnPoint.x, spawnPoint.y, color, desc)
            end
        else
...
function NPCSchedulerBridge.BroadcastTV(cx, cy)
    local cell = getCell()
    ...
    if #tvs then
        ...
        for _, line in pairs(lines) do
            ...
            NPCBroadcasterBridge.AddBroadcast(message)
        end
    end
end
...
function NPCSchedulerBridge.CheckEvent()
    if isServer() then return end

    local world = getWorld()
    local gamemode = world:getGameMode()

    -- Multiplayer spawns are owned by the server-side legacy world-director.
    -- Keep the old player-centered scheduler only for single-player legacy games.
    if gamemode == "Multiplayer" then return end
    ...
    if NPCUtils.GetCharacterID(currentPlayer) == NPCUtils.GetCharacterID(onlinePlayer) then

        -- SPAWN ATTACKING FORCE
        local daysPassed = currentPlayer:getHoursSurvived() / 24
        local waveData = NPCSchedulerBridge.GetWaveDataForDay(daysPassed)

        local densityScore = 1
        if bs_sandboxBool("General_DensityScore", true) then
           densityScore = NPCSchedulerBridge.GetDensityScore(currentPlayer, 120)
        end

        for _, wave in pairs(waveData) do
            local spawnZoneBoost = NPCSchedulerBridge.GetSpawnZoneBoost(currentPlayer, wave.clanId)
            ...
            if spawnRandom < spawnChance then
                NPCSchedulerBridge.SpawnWave(currentPlayer, wave)
            end
        end

        -- SPAWN DEFENDERS
        local spawnRandom = ZombRandFloat(0, 101)
        local spawnChance = tonumber(bs_sandboxValue("General_DefenderSpawnHourlyChanced", 8)) or 8
        if spawnRandom < spawnChance / 6 then
            NPCSchedulerBridge.SpawnDefenders(currentPlayer, 55, 100)
        end

        -- SPAWN BASES
        if bs_allowWorldDecoration() then
            local spawnRandom = ZombRandFloat(0, 101)
            local spawnChance = tonumber(bs_sandboxValue("General_BaseSpawnHourlyChance", 0)) or 0
            if spawnRandom < spawnChance / 6 then
                local sceneNo = 1 + ZombRand(#NPCBaseScenes)
                NPCSchedulerBridge.SpawnBase(currentPlayer, sceneNo)
            end
        end
    end
end

function NPCSchedulerBridge.Install()
    if Events and Events.EveryTenMinutes and Events.EveryTenMinutes.Add then
        Events.EveryTenMinutes.Add(NPCSchedulerBridge.CheckEvent)
    end
end
